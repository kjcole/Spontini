...
def orderPitchesInChord(text, lang):
    ...
    if numNotes > 1 and chordStartPos != -1 and chordEndPos != -1:
        ...
        ret = (
            text[: chordStartPos + 1]
            + pitchesOrdered[: len(pitchesOrdered) - 1]
            + text[chordEndPos:]
        )
...
def mergeTableCells(
    textFrom, fragmentOffsetStart, fragmentOffsetEnd, textTo, lang, debug=False
):
    ...
    for currTkn in iteratFrom.pitches():
        if isinstance(currTkn, ly.pitch.Pitch):
            ...
            if iteratFrom.position(currTkn) <= fragmentOffsetEnd + 1:
                fragSubstrEnd = (
                    iteratFrom.position(currTkn) + len(translatedTkn) - 2
                )  # it includes {
                # print("end: "+str(fragSubstrEnd))
            if fragSubstrStart == -1 and (
                iteratFrom.position(currTkn) + len(translatedTkn) - 1
            ) >= (fragmentOffsetStart + 1):
    ...
    if fragmentAttrs["isA"] == None:
        ret[
            "error"
        ] = "Can't do anything because textFrom fragment doesn't contain notes"
        return ret

    if textToAttrs["isA"] == None:
        ret[
            "error"
        ] = "Can't do anything because textTo doesn't contain notes/spacers/rests"
        return ret
    ...
    if textToAttrs["isA"] == "SPACER" or textToAttrs["isA"] == "REST":
        for currTkn in iteratTo.pitches():
            if textToAttrs["replaceStartPos"] == -1 and (
                isinstance(currTkn, ly.lex.lilypond.Spacer)
                or isinstance(currTkn, ly.lex.lilypond.Rest)
            ):
                textToAttrs["replaceStartPos"] = iteratTo.position(currTkn) - 1
                textToAttrs["replaceEndPos"] = (
                    iteratTo.position(currTkn) + len(currTkn) - 2
                )
    ...
    if textToAttrs["isA"] == "NOTES" and textToAttrs["notesNum"] == 1:
        for currTkn in iteratTo.pitches():
            if textToAttrs["replaceStartPos"] == -1 and isinstance(
                currTkn, ly.pitch.Pitch
            ):
                textToAttrs["replaceStartPos"] = iteratTo.position(currTkn) - 1
            if isinstance(currTkn, ly.pitch.Pitch):
                translatedTkn = translatePitches(currTkn.output(), lang)
                textToAttrs["replaceEndPos"] = (
                    iteratTo.position(currTkn) + len(translatedTkn) - 2
                )
    ...
    iteratFragment = ly.pitch.PitchIterator(srcFragment, language=lang)
    for currTkn in iteratFragment.pitches():
        if fragmentAttrs["copyStartPos"] == -1 and isinstance(currTkn, ly.pitch.Pitch):
            fragmentAttrs["copyStartPos"] = iteratFragment.position(currTkn) - 1
        if isinstance(currTkn, ly.pitch.Pitch):
            translatedTkn = translatePitches(currTkn.output(), lang)
            fragmentAttrs["copyEndPos"] = (
                iteratFragment.position(currTkn) + len(translatedTkn) - 2
            )
    ...
    if textFromAttrs["isA"] == "NOTES" and textFromAttrs["notesNum"] == 1:
        ret["textFrom"] = (
            textFrom[: textFromAttrs["replaceStartPos"]]
            + "s"
            + textFrom[textFromAttrs["replaceEndPos"] + 1 :]
        )
    # In case of chord, cut the fragment notes if their number is lower than the chord's notes
    elif (
        textFromAttrs["isA"] == "NOTES"
        and textFromAttrs["notesNum"] > 1
        and textFromAttrs["notesNum"] > fragmentAttrs["notesNum"]
    ):
        ret["textFrom"] = (
            textFrom[: textFromAttrs["replaceStartPos"]]
            + textFrom[textFromAttrs["replaceEndPos"] + 1 :]
    ...
    # If the number of notes of the fragment is the same of the notes in from, replace the notes with a spacer
        ...
        ret["textFrom"] = (
            textFrom[: textFromAttrs["startChordPos"] - 1]
            + "s"
            + textFrom[textFromAttrs["endChordPos"] + 1 :]
        )

    if fragmentAttrs["isA"] == "NOTES" and textToAttrs["isA"] == "NOTES":
        if textToAttrs["notesNum"] == 1:  # note
            ret["textTo"] = (
                textTo[: textToAttrs["replaceStartPos"]]
                + "< "
                + textTo[
                    textToAttrs["replaceStartPos"] : textToAttrs["replaceEndPos"] + 1
                ]
                + " "
                + textFromFragment
                + " >"
                + textTo[textToAttrs["replaceEndPos"] + 1 :]
            )
        else:  # chord
            ret["textTo"] = (
                textTo[: textToAttrs["replaceEndPos"]]
                + " "
                + textFromFragment
                + " >"
                + textTo[textToAttrs["replaceEndPos"] + 1 :]
    ...
    if fragmentAttrs["isA"] == "NOTES" and (
        (textToAttrs["isA"] == "SPACER") or (textToAttrs["isA"] == "REST")
    ):
...
def convertPitchesToEnharmonic(text, startOffs, endOffs, lang, debug=False):
    ...
    for currTkn in iterat.pitches():
        if isinstance(currTkn, ly.pitch.Pitch):
            ...
            if (
                currTkn.alter != 0
                and (iterat.position(currTkn) + len(translatedTkn) - 1) >= startOffs
                and iterat.position(currTkn) <= endOffs
            ):
...
def test():
    ...
    mergeTableCells(
        textFrom, fragmentOffsetStart, fragmentOffsetEnd, textTo, lang, True
    )
    ...
    ret = mergeTableCells(
        textFrom, fragmentOffsetStart, fragmentOffsetEnd, textTo, "italiano", True
    )
