...
def setSpontiniLogger(moduleName, redirect=None):
    ...
    formatter = ColoredFormatter(
        ...
        secondary_log_colors={"message": {"ERROR": "white", "CRITICAL": "red"}},
    )
...
def setPip3AndVenvParams(logFunc=mute):
    ...
    try:
        ...
        p = subprocess.run(
            pipcmdArr, encoding="utf-8", stderr=subprocess.PIPE, stdout=subprocess.PIPE
        )
        if p.returncode == 0:
            pip3Exists = True
            logFunc(p.stdout, "I")
        ...
        if pip3Exists:
            ...
            if venvCmdExists and os.path.isdir(venvDir):
                ...
                if not venvCmdExists:
                    logFunc(
                        "Could not find pip executable inside " + getVenvName(), "E"
                    )
                    logFunc(
                        "Make sure that both pip and venv are installed. If they are already installed, a broken installation",
                        "E",
                    )
                    logFunc(
                        "of the Spontini-Server could have been made. Spontini-Server will try to fix it at its next start.",
                        "E",
                    )
                    logFunc(
                        "Please run again the server and if the problem persists, report a bug.",
                        "E",
                    )
                    try:
    ...
    if pip3Exists and venvCmdExists:
        ...
        if venvExists:
            logFunc(
                "A virtual environment for python3 already exists inside " + venvDir,
                "I",
            )
        else:
            logFunc("", "I")
