...
configurableParams = [
    WORKSPACE_PARAM,
    VERSION_PARAM,
    CAN_CONFIG_FROM_NON_LOCALHOST_PARAM,
    FORK_ACCESS_ONLY_PARAM,
    DEBUG_PARAM,
    LILYPOND_EXEC_PARAM,
    INKSCAPE_EXEC_PARAM,
    MIDI_ENABLED_PARAM,
    DEFAULT_MIDI_INPUT_CHANNEL_PARAM,
    SOUNDFONT_URL_PARAM,
    DEFAULT_MODE_PARAM,
]
...
def executeScript(clientInfo, scriptFile):
    ...
    if scriptFile.endswith(".py"):
        loader = importlib.machinery.SourceFileLoader(
            scriptFile.replace(".py", ""), os.path.join(wsDirPath, scriptFile)
        )
        ...
        try:
            p = subprocess.run(
                [os.path.join(wsDirPath, scriptFile)],
                encoding="utf-8",
...
def getDefaultExecutableCmd(programInfo, errorLoggedAs="E"):
    ...
    ret = ""
    log(
        "Checking default " + programInfo["programName"] + " installation in sys path",
        "I",
    )
    if checkExecutable(programInfo["commonExecName"], errorLoggedAs):
        ...
        if not path:
            programFilesEnvX86 = os.environ.get(
            ...
            path = glob.glob(
                os.path.join(programFilesEnvX86, *programInfo["winSubPath"])
            )
            if path:
...
def checkIfIsChildFile(possibleChild, possibleParent, extension):
    ...
    ret = ret or (
        (possibleChild.startswith(possibleParent.replace(".ly", "") + "-"))
        and possibleChild.endswith(extension)
    )
    return ret
...
def removeFilteredTknFromOutputFiles(fileName):
    ...
    for currFile in os.listdir(wsDirPath):
        if (
            checkIfIsChildFile(currFile, fileName, ".svg")
            or checkIfIsChildFile(currFile, fileName, ".pdf")
            or checkIfIsChildFile(currFile, fileName, ".midi")
        ):
            currFileRenamed = currFile[::-1].replace("DERETLIF-", "", 1)[::-1]
            os.rename(
                os.path.join(wsDirPath, currFile),
                os.path.join(wsDirPath, currFileRenamed),
            )
    try:
...
def doPostSync(message, request):
    ...
    if message["cmd"] == "DELETE":
        ...
        for currFile in sorted(os.listdir(wsDirPath)):
            if (
                inputFileNameWOSuffix + "-afterpdf" in currFile
                or inputFileNameWOSuffix + "-beforepdf" in currFile
            ):
                try:
        ...
        for currFile in sorted(os.listdir(wsDirPath)):
            if currFile == inputFileNameWOSuffix + ".svg":
                os.remove(os.path.join(wsDirPath, currFile))
            if currFile.startswith(inputFileNameWOSuffix + "-") and currFile.endswith(
                ".svg"
            ):
                os.remove(os.path.join(wsDirPath, currFile))
    ...
    if message["cmd"] == "COMPILE":
        ...
        if "param5" in message and param3 != "null":
            ...
            if (
                (message["param5"] != "")
                and (message["param5"] != "empty")
                and (message["param5"] != message["param2"])
            ):
                filtered = True
        ...
        if param3.strip() != "pdf":
            for currFile in os.listdir(wsDirPath):
                if checkIfIsChildFile(
                    currFile, inputFileName, ".svg"
                ) or checkIfIsChildFile(currFile, inputFileNameNotFiltered, ".svg"):
                    os.rename(
                        os.path.join(wsDirPath, currFile),
                        os.path.join(
                            wsDirPath, currFile.replace(".svg", ".ljssvgswap")
                        ),
                    )
        ...
        try:
            ...
            if p.returncode == 0:
                for currFile in os.listdir(wsDirPath):
                    if checkIfIsChildFile(currFile, inputFileName, ".ljssvgswap"):
                        try:
                            ...
                            pass
                    if checkIfIsChildFile(
                        currFile, inputFileNameNotFiltered, ".ljssvgswap"
                    ):
                        try:
                            os.remove(os.path.join(wsDirPath, currFile))
                        except:
                ...
                if param3.strip() == "pdf":
                    try:
                        ...
                        pdf = PdfFileReader(
                            open(os.path.join(wsDirPath, pdfFileName), "rb")
                        )
                ...
                if "param4" in message:
                    outLines = "\n".join(
                        [s for s in outLines.split("\n") if message["param4"] in s]
                    )
                log(clientInfo + outLines, "S")
                log(
                    clientInfo + "Compilation time: " + str(endTime - startTime)[:-4],
                    "I",
                )
            else:
                for currFile in os.listdir(wsDirPath):
                    if checkIfIsChildFile(currFile, inputFileName, ".ljssvgswap"):
                        os.rename(
                            os.path.join(wsDirPath, currFile),
                            os.path.join(
                                wsDirPath, currFile.replace(".ljssvgswap", ".svg")
                            ),
                        )
                status = "KO"
                log(clientInfo + p.stdout, "E")
            ...
            for currFile in sorted(os.listdir(wsDirPath)):
                if (
                    inputFileNameNotFiltered.replace(".ly", "") + "-afterpdf"
                    in currFile
                ):
                    log(
                        clientInfo
                        + "[generating PDF] executing : "
                        + currFile
                        + " script",
                        "I",
                    )
            ...
            if not checkExecutable(lilyExecutableCmd):
                log(
                    clientInfo + "Lilypond called, but its executable file is not set!",
                    "E",
                )
    ...
    if message["cmd"] == "SET_WORKSPACE":
        if not canConfigFromNonLocalhost and (
            (not "localhost" in host) and (not "127.0.0.1" in host)
        ):
                        ...
                        if displayedName == pluginName:
                            ...
                            try:
                                ...
                                if ("127.0.0.1" in access) or ("localhost" in access):
                                    if (not "localhost" in host) and (
                                        not "127.0.0.1" in host
                                    ):
                                        return sendCompleteResponse("KO", "Not allowed")
                            except:
                                pass
                            ...
                            try:
                                outputTempFileName = (
                                    outputTempFileName
                                    + "."
                                    + plugin.find("output-file-extension").text
                                )
                            except:
                                pass
                            ...
                            try:
                                inputTempFileName = (
                                    inputTempFileName
                                    + "."
                                    + plugin.find("input-file-extension").text
                                )
                            except:
                                pass
                            ...
                            try:
                                ...
                                try:
                                    tmpfin.write(
                                        plugin.find("input-file-pre-token").text.encode(
                                            "utf-8"
                                        )
                                    )
                                except:
                                ...
                                res = (
                                    "Can't create temporary input file before executing command: "
                                    + command
                                )
                                log(clientInfo + traceback.format_exc(), "E")
                                gotError0 = True
                            ...
                            if gotError0 == False:
                                try:
                                    for i in range(len(commandArr)):
                                        ...
                                        pluginsAbsPath = os.path.abspath(
                                            os.path.join("..", "plugins")
                                        )
                                        if commandArr[i].startswith(
                                            "%%PLUGINS_ABS_PATH%%"
                                        ):
                                            commandArr[i] = commandArr[i].replace(
                                                "%%PLUGINS_ABS_PATH%%", pluginsAbsPath
                                            )
                                        ...
                                        if commandArr[i].startswith("%%VENV%%"):
                                            ...
                                            commandArr[i] = os.path.join(
                                                venvedExecDir, commandArr[i]
                                            )

                                        if commandArr[i].startswith("%%LILYINSTDIR%%"):
                                            lilyInstDir = str(
                                                pathlib.Path(lilyExecutableCmd).parent
                                            )
                                            commandArr[i] = commandArr[i].replace(
                                                "%%LILYINSTDIR%%", ""
                                            )
                                            commandArr[i] = os.path.join(
                                                lilyInstDir, commandArr[i]
                                            )
                                    ...
                                    if (
                                        "python -m" in " ".join(commandArr)
                                        and "No module named" in p.stderr
                                    ):
                                        missingInstallation = True

                                except FileNotFoundError:
                                    missingInstallation = True
                                    res = "Error while executing command: " + " ".join(
                                        commandArr
                                    )
                                ...
                                if missingInstallation:
                                    try:
                                        koToken = (
                                            "_" + plugin.find("install-command").text
                                        )
                                        koToken = koToken.replace(
                                            "%%VENVEDPYTHON3%%", venvedPyCmd
                                        )
                                    except:
                                        pass
                                ...
                                try:
                                    os.remove(
                                        os.path.join(wsDirPath, inputTempFileName)
                                    )
                                except:
                                    log(
                                        clientInfo
    ...
    if message["cmd"].startswith("CREATE_"):
        ...
        if "ZIP" in message["cmd"]:
            ...
            for currFile in sorted(os.listdir(wsDirPath)):
                if (
                    (currFile == fileNameWOSuffix + ".svg")
                    or (
                        currFile.startswith(fileNameWOSuffix + "-")
                        and currFile.endswith(".svg")
                    )
                    or (currFile == fileName)
                    or (currFile == fileNameWOSuffix + "-svgexport.pdf")
                    or (fileNameWOSuffix + "-afterpdf" in currFile)
                    or (fileNameWOSuffix + "-beforepdf" in currFile)
                    or (currFile == fileNameWOSuffix + ".pdf")
                    or (currFile == fileNameWOSuffix + ".midi")
                ):
                ...
                try:
                    ...
                    p = subprocess.run(
                        [
                            ...
                            svgFile,
                            "--export-text-to-path",
                            "--export-filename=" + fileWOSuffix + ".pdf",
                        ],
                        encoding="utf-8",
                        stderr=subprocess.STDOUT,
                        stdout=subprocess.PIPE,
                    ...
                    if not checkExecutable(inkscapeExecutableCmd):
                        log(
                            clientInfo
                            + "Inkscape called, but its executable file is not set!",
                            "E",
                        )
                        return sendCompleteResponse(
                            status, "Inkscape executable not found".encode("utf8")
                        )
                    else:
                        log(clientInfo + traceback.format_exc(), "E")
            ...
            if len(pdfList) > 1:
                log(
                    clientInfo
                    + "[generating PDF] Concatenating all the pages into one PDF file...",
                    "I",
                )
            for pdf in pdfList:
                merger.append(pdf)
            ...
            os.rename(
                os.path.join(wsDirPath, fileNameWOSuffix + "-cpy.pdf"),
                os.path.join(wsDirPath, fileNameWOSuffix + "-svgexport.pdf"),
            )
            log(
                clientInfo
                + "[generating PDF] Generated: "
                + os.path.join(wsDirPath, fileNameWOSuffix + "-svgexport.pdf"),
                "S",
            )
            ...
            try:
                ...
                for plugin in root.findall("plugin"):
                    try:
                        ...
                        currPlugin = (
                            displayedName + sep2 + appInput + sep2 + appOutput + sep2
                        )
                        currPlugin = (
                            currPlugin
                            + command
                        ...
                        addVenvWarn = False
                        venvPySomewhere = (
                            "%%VENVEDPYTHON3%%" in command
                            or "%%VENVEDPYTHON3%%" in winCommand
                        )
                        venvPySomewhere = (
                            venvPySomewhere
                            or "%%VENVEDPYTHON3%%" in installPluginCommand
                        )
                        ...
                        log(
                            clientInfo
                            + "Badly formatted <plugin> element in plugins.xml file: ",
                            "E",
                        )
                        log(clientInfo + traceback.format_exc(), "E")
                        plugins = ""
    ...
    if message["cmd"] == "NUM_SCORE_PAGES":
        ...
        if fileType == "svg":
            ...
            try:
                pdf = PdfFileReader(
                    open(os.path.join(wsDirPath, fileName + ".pdf"), "rb")
                )
    ...
    if message["cmd"] == "MERGE_TABLE_CELLS":
        ...
        try:
            ...
            mergeRet = mergeTableCells(
                textFrom, fragmentOffsetStart, fragmentOffsetEnd, textTo, language
            )
            if mergeRet["error"] == "":
    ...
    if message["cmd"] == "CONVERT_PITCHES_TO_ENHARMONIC":
        ...
        try:
            ...
            ret = convertPitchesToEnharmonic(
                text, fragmentOffsetStart, fragmentOffsetEnd, language
            )
        except:
            status = "KO"
        return sendCompleteResponse(status, ret)
...
if not os.path.isdir(wsDirPath):
    if wsDirPath == "":
        ...
        log(
            ...
            + wsDirPath
            + "' not found: setting workspace to default 'examples' dir",
            "W",
        )
