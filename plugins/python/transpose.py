...
spontini_utils_path = os.path.abspath(
...
def main():
    ...
    if len(argv) != 5:
        print(
            "Usage: python3 " + argv[0] + " input_params language inputfile outputfile",
            file=sys.stderr,
        )
        return 1
    ...
    try:
        ...
        p = subprocess.run(
            cmd, encoding="utf-8", stderr=subprocess.PIPE, stdout=subprocess.PIPE
        )
        ...
        cmd = [
            ...
            "-l",
            language,
            "transpose " + inputParamsInNederlands,
            "-o",
            outFileName,
            inFileName,
        ]
        p = subprocess.run(
            cmd, encoding="utf-8", stderr=subprocess.STDOUT, stdout=subprocess.PIPE
        )
