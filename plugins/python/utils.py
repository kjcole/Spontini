...
def checkAndGetExecutableWithPath(
    execName, winExecName, pathAfterProgramFiles, testOption
):
    ...
    if os.name == "nt":
        ...
        if which(winExecName) is not None:
            ...
            path = glob.glob(
                os.path.join(
                    programFilesEnv, pathAfterProgramFiles[0], pathAfterProgramFiles[1]
                )
            )
            if not path:
                ...
                path = glob.glob(
                    os.path.join(
                        programFilesEnvX86,
                        pathAfterProgramFiles[0],
                        pathAfterProgramFiles[1],
                    )
                )
                if path:
...
def readSharedInfo(infoFileName):
    ...
    try:
        path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "shared", infoFileName)
        )
