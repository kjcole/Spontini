...
wsThread = None
webServerProc = None
webserverScript = os.path.join(
    os.path.dirname(__file__), "lib", "python", "spontini_server_webserver.py"
)

sendMsgScript = os.path.join(os.path.dirname(__file__), "lib", "python", "send_msg.py")
...
def getWebserverParam(param):
    try:
        path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "lib", "python", "protocolAndPort")
        )
        fp = open(path, "r")
        ret = ""
        if param == "protocol":
...
def message(txt, loggerTag=None, startTagIdx=0, endTagIdx=0, startMsgIdx=0):
    global textArea
    if not textArea:
        return
...
def initVenvWithModules():
    ...
    if not error:
        ...
        if not error:
            ...
            requirementsFile = os.path.abspath(
                os.path.join(
                    os.path.dirname(__file__), "lib", "python", "requirements.txt"
                )
            )

            pip3VenvCmd = os.path.join(pipAndVenvParams[2], "pip3")
            installedModules = subprocess.check_output(
                [pip3VenvCmd, "freeze"], encoding="utf-8"
            )
...
def WebServerThread():
    ...
    webServerProc = subprocess.Popen(
        webserverCLI, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
    )
    while True:
...
def sendMsgToServer(msg):
    ...
    if webServerURL == "":
        ...
        showerror(
            title="Error", message="Not enabled on this GUI: please set it through CGI"
        )
        return
    newCLI = sendMsgCLI[:]
...
if "nogui" in argv:
    ...
    logoFilePath = os.path.join(
        os.path.dirname(__file__), "lib", "webgui", "images", "logo.png"
    )
